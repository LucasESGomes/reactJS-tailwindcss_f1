...
function Header() {
  ...
  return (
      <header className="text z-10 flex bg-black justify-between items-center text-white px-8 md:px-32 drop-shadow-md top-0 left-0 w-full h-20">
        ...
        <div
          className={`absolute xl:hidden top-24 left-0 w-full bg-white flex flex-col items-center gap-6 font-semibold
           text-lg transform transition-transform ${
             isMenuOpen ? "opacity-100" : "opacity-0"
           }`}
          style={{ transition: "trasform 0.3s ease, opacity 0.3s ease" }}
        >
          <li className="list-none w-full text-center p-4 hover:bg-sky hover:text-white transition-all cursor-pointer">
            Inicío
          </li>
          <li className="list-none w-full text-center p-4 hover:bg-sky hover:text-white transition-all cursor-pointer">
            Carros
